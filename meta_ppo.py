...
class Meta_PPO:
	def __init__(self, policy_class, loss_fn, env, **hyperparameters):
		...
		self.loss_fn_optim = Adam(self.loss_fn.parameters(), lr=self.alph)
	...
	def learn(self, total_timesteps):
		...
		while elapsed_timesteps < total_timesteps:                                                                      
			...
			if elapsed_iterations % self.save_freq == 0:
				torch.save(self.actor.state_dict(), 'models/meta_ppo_actor.pth')
				torch.save(self.critic.state_dict(), 'models/meta_ppo_critic.pth')
				torch.save(self.loss_fn.state_dict(), 'models/meta_ppo_loss_fn.pth')
	...
	def rollout(self):
		...
		while t < self.timesteps_per_batch:
			...
			for ep in range(self.timesteps_per_episode):
				...
				# to-do: use loss function to calc loss based on 1 obs
